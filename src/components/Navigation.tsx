...
import { config, getWhatsAppUrl, getPhoneUrl } from '@/lib/config';
...
export function Navigation() {
  ...
  const handleNavClick = useCallback(
    (e: React.MouseEvent, item: typeof navItems[0]) => {
      ...
      } else if (item.href !== location.pathname && item.sectionId) {
    ...
    <>
      <header
        className={cn(
          ...
          isScrolled
            ? 'bg-background/95 backdrop-blur-lg shadow-sm py-4'
            : 'bg-transparent py-6'
        )}
      >
        <nav className="container-wide flex items-center justify-between">
          ...
          <div className="hidden lg:flex items-center gap-8">
            {navItems.map((item) => (
              <Link
                ...
                className={cn(
                  'link-underline text-sm font-medium tracking-wide uppercase transition-colors',
                  (isHomePage ? activeSection === item.sectionId : location.pathname === item.href)
                    ? 'text-primary'
          ...
          <div className="hidden lg:flex items-center gap-4">
            ...
            >
              ...
              <span className="hidden xl:inline">{config.contact.phone}</span>
            </a>
            <a
              href={getWhatsAppUrl()}
              target="_blank"
